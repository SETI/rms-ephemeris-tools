...
def draw_planetary_view_mpl(
    ...
    # ------------------------------------------------------------------
    # Build matplotlib figure (extra bottom space for full caption block)
    # ------------------------------------------------------------------
    nc = max(0, int(options.ncaptions))
    caption_extra = max(0.0, 0.024 * max(0, nc - 1))
    fig_bottom = min(0.42, 0.17 + caption_extra)
    fig_height_in = 8.5 + min(1.2, caption_extra * 2.5)
    fig, ax = figure_and_axes(
        fig_width_in=8.0,
        fig_height_in=fig_height_in,
        left=0.14, right=0.91, top=0.88, bottom=fig_bottom,
    )
...
def _add_radec_ticks(
    ...
    Tick step sizes follow ``_rspk_labels2`` / ``_STEP1``; labels use ``_rspk_format_label``.
    """
    ...
    for k in range(k1_ra, k2_ra + 1):
        ...
        if abs(x) <= delta:
            ...
            tick_lines_y.append([-delta + length, -delta])
            if ismajor:
                label = _rspk_format_label(s, 'B')
                # Bottom label (FOV ymin is the bottom of the plot after inverted ylim)
                ax.text(
                    x, -delta - dtick2 * 1.5,
                    label,
    ...
    for k in range(k1_dec, k2_dec + 1):
        ...
        y = -cam[1] / cam[2]
        if abs(y) <= delta:
            # Left edge (FOV xmin = screen left, same as PostScript _rspk_labels2)
    ...
    ax.set_xlabel('Right Ascension (h m s)', fontsize=9, labelpad=14)
    # Declination label on the LEFT; labelpad must clear margin tick labels.
    ax.set_ylabel('Declination (d m s)', fontsize=9, labelpad=40)
...
def _add_captions(
    ...
    # Place captions below the axes (same rows as PS / DrawPlanetaryViewOptions).
    ax_pos = ax.get_position()
    n = min(ncaptions, len(lcaptions))
    y_top_cap = ax_pos.y0 - 0.055
    y_floor = 0.03
    avail = max(0.02, y_top_cap - y_floor)
    line_h = min(0.026, avail / max(n, 1))
    line_h = max(0.014, line_h)
    for i in range(n):
        y = y_top_cap - i * line_h
...
def _add_moon_labels(
    ...
    consistent with ``finalize`` limits (x and y match the PostScript viewport).
    ...
    for mname, mx, my in sorted_by_dist:
        # Moon dot in axes-relative screen points (origin bottom-left of axes box).
        sx = (mx + delta) / (2.0 * delta) * ax_w_pt
        sy = (delta - my) / (2.0 * delta) * ax_h_pt
