...
c***********************************************************************
        ...
        parameter               (PLANET_NAME = 'Jupiter')
        parameter               (STARLIST = 'starlist_jup.txt')
        parameter               (LONDIR = 'W')

        double precision        RPLANET
        parameter               (RPLANET = 71492.d0)
...
c Ring constants
     ...
     &                                  221900.d0, 221900.d0,
     &                                  422000.d0 /
        double precision        ring_elevs(NRINGS)/
     &                                     0.d0,      0.d0,
     &                                  1160.d0,  -1160.d0,
     &                                  4310.d0,  -4310.d0,
     &                                     0.d0 /
     ...
     &                                                  NRINGS * 0.d0 /
        ...
        logical                 ring_flags(NRINGS)/ 7*.FALSE. /
        logical                 ring_opaqs(NRINGS)/ 7*.FALSE. /
        logical                 ring_dashed(NRINGS)
     &                                  / 2*.FALSE., 5*.TRUE./
...
c Planet-specific declarations
        double precision        planet_time, reflon
        double precision        rotmat(3,3), eq_pole(3),
     &                          j2000_pole(3), ascnode(3), tempvec(3)
...
c Ring selection
        ...
        status = WWW_GetKey('torus', string)
        if (status) then
            status = WWW_GetKey('torus_inc', string)
            status = WWW_GetKey('torus_rad', string2)
            write(*,10) '          Io torus: Inclination = ' //
     &                  string(1:LASTNB(string)) //
     &                  ' deg; Radius = ' // 
     &                  string2(1:LASTNB(string2)) // ' km'
        else
            write(*,10) '          Io torus: No'
        end if
...
c Get the field of view units
     ...
     &           string(1:2) .eq. 'km') then
            ...
            fov = fov * 7.292d-3
        else if (index(string, 'Galileo') .gt. 0) then
            fov = fov * 8.1d-3
            ...
            fov = fov * 6.d-3
        else if (index(string, 'UVIS slit') .gt. 0) then
            fov = fov * 59.d-3
        else
            goto 9999
        end if
...
c Check for a ring
            ...
            if (index(string,'Halo') .gt. 0) then
                radius = ring_rads(1)
            else if (index(string,'Main') .gt. 0) then
                radius = ring_rads(2)
            else if (index(string,'Amalthea') .gt. 0) then
                radius = ring_rads(4)
            else if (index(string,'Thebe') .gt. 0) then
                radius = ring_rads(6)
            else
                goto 9999
            end if
...
c***********************************************************************
        ...
        if (index(string,'Main') .gt. 0) then
                ring_flags(1) = .TRUE.
                ring_flags(2) = .TRUE.
        end if

        if (index(string,'Gossamer') .gt. 0) then
                ring_flags(3) = .TRUE.
                ring_flags(4) = .TRUE.
                ring_flags(5) = .TRUE.
                ring_flags(6) = .TRUE.
...
c***********************************************************************
c Set up the Io torus if necessary
c***********************************************************************

        keyword = 'TORUS_SELECTION'
        status = WWW_GetKey('torus', string)
        if (status) then
                ring_flags(7) = .TRUE.
        end if

        if (ring_flags(7)) then
            keyword = 'TORUS_INCLINATION'
            status = WWW_GetKey('torus_inc', string)
            read(string, *, err=9999) value
            ring_incs(7) = value * RPD()

            keyword = 'TORUS_RADIUS'
            status = WWW_GetKey('torus_rad', string)
            if (ring_rads(7) .le. 0.) goto 9999

c Calculate the instantaneous position of the observer
            call RSPK_ObsLoc(obs_time, obs_pv)

c Calculate the back-dated position of Mars (neglecting aberration)
            call SPKAPP(PLANET_ID, obs_time, 'J2000', obs_pv, 'LT',
     &                  planet_dpv, dt)
            planet_time = obs_time - dt

c Note that the matrix returned by BODMAT converts from J2000 to the 
c body equator/prime meridian frame (System III).  Its transpose does
c the reverse, which means that its three columns are the body principal
c axes in J2000 coordinates.
            call BODMAT(PLANET_ID, planet_time, rotmat)
            call VPACK(rotmat(3,1), rotmat(3,2), rotmat(3,3), eq_pole)

c Calculate equator's J2000 ascending node in Jupiters' System III
c equator/prime meridian coordinates
            call VPACK(0.d0, 0.d0, 1.d0, j2000_pole)
            call VCRSS(j2000_pole, eq_pole, ascnode)
            call MXV(rotmat, ascnode, tempvec)
            reflon = atan2(tempvec(2), tempvec(1))
        
c The magnetic pole is at longitude 202 in System III.  Longitudes
c increase westward for Jupiter so this is 158 in a positive sense,
c The ascending node is 90 ahead of this, at longitude 248.
            ring_nodes(7) = RPD() * 248.d0 - reflon
        end if
...
81      format(f7.3,'\260')
        ...
        if (status) then
            ncaptions = ncaptions + 1
            lcaptions(ncaptions) = 'Io torus:'

            status = WWW_GetKey('torus_inc', string)
            status = WWW_GetKey('torus_rad', string2)
            rcaptions(ncaptions) = 'Inclination = ' //
     &                  string(1:LASTNB(string)) //
     &                  ' deg; Radius = ' // 
     &                  string2(1:LASTNB(string2)) // ' km'
        end if
