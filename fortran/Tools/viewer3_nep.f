...
c***********************************************************************
        ...
        parameter               (PLANET_NAME = 'Neptune')
        parameter               (STARLIST = 'starlist_nep.txt')
        parameter               (LONDIR = 'W')

        double precision        RPLANET
        parameter               (RPLANET = 24764.d0)
...
c Ring constants
        ...
        double precision        ring_rads(NRINGS)/ 42000.d0,
     &                            53200.d0, 57500.d0, 62932.d0 /
     ...
     &                                                  NRINGS * 0.d0 /
        ...
        logical                 ring_flags(NRINGS)/ .FALSE.,
     &                               .TRUE., .FALSE., .TRUE. /

        logical                 ring_opaqs(NRINGS)/ 4 * .FALSE. /

        logical                 ring_dashed(NRINGS)/ .TRUE.,
     &                                 .FALSE., .TRUE., .FALSE. /

        integer                 NARCS
        parameter               (NARCS = 5)
        integer                 arc_rings(NARCS)/ 5 * 4 /
        double precision        arc_minlons(NARCS)/ 247.1,
     &                              261.1, 264.9, 275.7, 284.5 /
        double precision        arc_maxlons(NARCS)/ 256.7,
     &                              264.1, 265.9, 279.8, 285.5 /
        double precision        arc_widthpts
        logical                 arc_flags(NARCS)/ 5 * .TRUE. /
        double precision        arc_motion(3)/ 820.1194d0, 820.1118d0,
     &                                         820.1121d0 /
        character*12            arc_names(NARCS)/   '  Fraternite',
     &                              '   Egalite B', '   Egalite A',
     &                              '     Liberte', '     Courage' /

c Planet-specific declarations
        double precision        ref_time, ddays, minlon, maxlon
        integer                 imodel
        double precision        FJUL_TAIofJD
...
c Field of view
     ...
     &          string = '1'
        write(*,10) '   Field of view: ' // string(1:LASTNB(string)) //
...
c Arc model
        status = WWW_GetKey('arcmodel', string)
        write(*,10) '       Arc model: ' // string(1:LASTNB(string))
...
c Other bodies
        ...
        if (.not. status) then
            ...
            write(*,10) '    Other bodies: ' // string(1:LASTNB(string))
            do 50 i = 1, 9999
                status = WWW_GetKeys('other', string)
...
c Arc points
        status = WWW_GetKey('arcpts', string)
        write(*,10) '      Arc weight: ' // string(1:LASTNB(string))
     &                  // ' (points)'
...
c Select arc motion model
        keyword = 'ARC_MODEL'
        status = WWW_GetKey('arcmodel', string)

        if (index(string,'#1') .gt. 0) then
                imodel = 1
        else if (index(string,'#2') .gt. 0) then
                imodel = 2
        else if (index(string,'#3') .gt. 0) then
                imodel = 3
        else
                goto 9999
        end if
...
c Check for a ring
            ...
            if (index(string,'Galle') .gt. 0) then
                radius = ring_rads(1)
            else if (index(string,'LeVerrier') .gt. 0) then
                radius = ring_rads(2)
            else if (index(string,'Arago') .gt. 0) then
                radius = ring_rads(3)
            else if (index(string,'Adams') .gt. 0) then
                radius = ring_rads(4)
            else
                goto 9999
            end if
...
c LeVerrier and Adams are always selected by default

        if (index(string,'Galle') .gt. 0) ring_flags(1) = .TRUE.
        if (index(string,'Arago') .gt. 0) ring_flags(3) = .TRUE.

c Reference epoch is JED 2447757.0
        ref_time = FJUL_ETofTAI( FJUL_TAIofJD(2447757.0d0,2) )
...
c Update arc longitudes (convert from deg B1950 to rad J2000)
        do 300 i = 1, NARCS
            arc_minlons(i) = arc_minlons(i) +
     &                          arc_motion(imodel) * ddays
            arc_maxlons(i) = arc_maxlons(i) +
     &                          arc_motion(imodel) * ddays

            arc_minlons(i) = RPD() * (arc_minlons(i) + 0.334321d0)
            arc_maxlons(i) = RPD() * (arc_maxlons(i) + 0.334321d0)
300     continue
...
c Write arc locations
        write(*,*) ' '
        do i = NARCS, 1, -1
                minlon = mod(arc_minlons(i) / RPD(), 360.d0)
                if (minlon .lt. 0.d0) minlon = minlon + 360.d0

                maxlon = mod(arc_maxlons(i) / RPD(), 360.d0)
                if (maxlon .lt. 0.d0) maxlon = maxlon + 360.d0

                if (i .eq. NARCS) then
                    write(*,34) arc_names(i), minlon, maxlon,
     &                          'from ring plane ascending node'
                else
                    write(*,34) arc_names(i), minlon, maxlon
                end if
34              format(1x, 1x, a12, ' longitude (deg): ',
     &                 f9.5, '  to ', f9.5, 2x, a)
        end do 
