...
c***********************************************************************
        ...
        parameter               (PLANET_NAME = 'Mars')
        parameter               (STARLIST = 'starlist_mar.txt')
        parameter               (LONDIR = 'W')

        double precision        RPLANET
        parameter               (RPLANET = 3397.d0)

c Moon parameters
        integer                 NMOONS
        parameter               (NMOONS = 2)
        integer                 MOON_IDS(0:NMOONS)/ 499, 401, 402 /
...
c Ring constants
        ...
        double precision        ring_rads(NRINGS)/ 2*9378.d0,
     &                                             2*23459.d0 /
        double precision        ring_elevs(NRINGS)/ -177.d0, 177.d0,
     &                                             -2656.d0, 2656.d0 /
        double precision        ring_eccs(NRINGS)/ NRINGS * 0.d0 /
        double precision        ring_incs(NRINGS)/ 2*0.d0,
     &                                             2*0.11346d0 /
        double precision        ring_peris(NRINGS)/ NRINGS * 0.d0 /
        double precision        ring_nodes(NRINGS)/ NRINGS * 0.d0 /
        double precision        ring_shifts(NRINGS)/ -3398.d0,
     &                                               -3398.d0,
     &                                              2*6796.d0 /
     ...
     &                                                  NRINGS * 0.d0 /
        ...
        logical                 ring_flags(NRINGS)/ NRINGS * .TRUE. /

        logical                 ring_opaqs(NRINGS)/ NRINGS * .FALSE. /

        logical                 ring_dashed(NRINGS)/ .FALSE., .FALSE.,
     &                                               .TRUE.,  .TRUE. /
...
c Planet-specific declarations
        double precision        planet_time, planet_pv(6), antisun(3)
        double precision        rotmat(3,3), orbit_pole(3), eq_pole(3),
     &                          j2000_pole(3), ascnode(3), tempvec(3)
        double precision        reflon, nodelon
        integer                 iring
...
c***********************************************************************
c Solve for the node of the Deimos ring
c***********************************************************************
...
c Calculate the instantaneous orbit pole from Mars' position/velocity
c relative to the Sun
        call SPKEZ(PLANET_ID, planet_time, 'J2000', 'NONE', SUN_ID,
     &          planet_pv, dt)
        call VCRSS(planet_pv(1), planet_pv(4), orbit_pole)
...
c Calculate equator's J2000 ascending node, in Mars' equator/prime
c meridian coordinates
...
c Locate the equator's ascending node on the orbit plane, in
c body equator/prime meridian coordinates
        call VCRSS(orbit_pole, eq_pole, ascnode)
        call MXV(rotmat, ascnode, tempvec)
        nodelon = atan2(tempvec(2), tempvec(1))

c Set Deimos ring node longitudes
        ring_nodes(3) = nodelon - reflon
        ring_nodes(4) = ring_nodes(3)

c***********************************************************************
c Solve for the center offset of each ring
c***********************************************************************

c Calculate equatorial projection of anti-Sun direction
        call VPERP(planet_pv(1), eq_pole, antisun)
        call VHAT(antisun, antisun)

c Find the offset vector from Mars' center for each ring
        do iring = 1, NRINGS
                call VSCL(ring_shifts(iring), antisun,
     &                          ring_offsets(1,iring))
        end do
...
c Both rings are selected by default
        if (index(string,'None') .gt. 0) then
                ring_flags(1) = .FALSE.
                ring_flags(2) = .FALSE.
                ring_flags(3) = .FALSE.
                ring_flags(4) = .FALSE.
