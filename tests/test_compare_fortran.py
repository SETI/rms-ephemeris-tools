...
def test_compare_tables_reports_max_abs_diff() -> None:
    """compare_tables reports the largest numeric absolute difference."""
    ...
    try:
        ...
        assert 'max_abs_diff=1' in res.message
    finally:
        path_a.unlink()
        path_b.unlink()


def test_compare_tables_ignores_fortran_overflow_token() -> None:
    """FORTRAN overflow fields ('*****') are ignored in comparison."""
...
def test_compare_postscript_ignores_generated_footer_timestamp() -> None:
    """compare_postscript ignores generated footer time-line jitter."""
    ps_a = (
        '%!PS-Adobe-2.0\n'
        '0 0 moveto\n'
        '(Generated by the Jupiter Tracker Tool, PDS Ring-Moon Systems Node, '
        'Mon Feb 16 12:39:51 2026)\n'
        'show\n'
        '%%EOF\n'
    )
    with tempfile.NamedTemporaryFile(mode='w', suffix='.ps', delete=False) as f:
