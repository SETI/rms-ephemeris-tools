...
from ephemeris_tools.viewer import run_viewer, _propagated_saturn_f_ring
from ephemeris_tools.planets import SATURN_CONFIG
from ephemeris_tools.time_utils import tdb_from_tai, tai_from_day_sec
...
class TestHighPriorityDifferences:
    ...
    def test_radec_offset_units_earth_observer(self) -> None:
        ...
        # Clear and reload SPICE to ensure Jupiter kernels are loaded
        import cspyce
        cspyce.kclear()
        ...
        assert 'dRA (deg)' not in txt_content or txt_content.count('dRA (deg)') < txt_content.count('dRA (")'), \
            "Earth observer should primarily use arcsec, not degrees"

    @pytest.mark.skip(reason="Requires spacecraft SPICE kernels")
    def test_radec_offset_units_spacecraft_observer(self) -> None:
        ...
        This test is skipped by default as it requires spacecraft SPICE kernels.
        """
        # Load Saturn kernels with Cassini spacecraft
        ...
        header_line = [line for line in lines if 'dRA' in line and 'dDec' in line][0]
