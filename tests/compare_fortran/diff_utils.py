...
def compare_postscript_images(
    ...
    *,
    ...
    if ignore_axis_pixels:
        ...
        footer_search_start = int(round(0.92 * h))
        ...
        if row_candidates.size >= 2 and col_candidates.size >= 2:
            ...
            footer_search_start = max(
                footer_search_start,
                bottom + tick_pad + band + max(4, int(round(dpi / 8))),
            )
            details.append(
                '  Axis mask: enabled '
        ...
        footer_row_thresh = max(10, int(round(0.01 * w)))
        ...
        if footer_candidates.size > 0:
            ...
            footer_pad = max(4, int(round(dpi / 20)))
            ...
            footer_rows = max(25, int(round(dpi / 4)))
            ignore_mask[h - footer_rows : h, :] = True
            details.append(
                f'  Footer mask: fallback bottom {footer_rows} rows '
                '(Generated by...)'
            )
